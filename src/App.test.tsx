...
describe('landing page', () => {
  it('renders the essential conversion content', () => {
    ...
    expect(heroVideo).toHaveAttribute('poster', '/assets/hero-desktop-poster.webp')
    expect(heroVideo).toHaveProperty('muted', true)
    const sources = heroVideo.querySelectorAll('source')
    expect(sources[0]).toHaveAttribute('src', '/assets/hero-mobile.webm')
    expect(sources[0]).toHaveAttribute('media', '(max-width: 680px)')
    expect(sources[2]).toHaveAttribute('src', '/assets/hero-desktop.webm')
    expect(sources[3]).toHaveAttribute('src', '/assets/hero-desktop.mp4')
